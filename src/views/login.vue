...
import { login,getcode } from '../api/userMG'
import { setCookie, getCookie, delCookie } from '../utils/util'
import Identify from './code'
          ...
          login(this.ruleForm).then(res => {
            ...
            if (res.errorCode==200) {
              ...
              setTimeout(() => {
                ...
                console.log(res.jur)
                localStorage.setItem('jur', JSON.stringify(res.jur))
                localStorage.setItem('userdata', JSON.stringify(res.date[0]))
                this.$store.commit('login', 'true')
                this.$router.push({ path: '/user/findAll' })
              }, 1000)
            } else {
